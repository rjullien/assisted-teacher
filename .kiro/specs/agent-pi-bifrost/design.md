...
| D13 | Confiance projet (`defaultProjectTrust`) | Fixée **explicitement** à `never` dans `settings.json`, plus `--no-approve` par exécution, et `--no-context-files` | Laisser la valeur par défaut `ask` | Les modes non interactifs (`-p`, `--mode json`, `--mode rpc`) n'affichent aucune invite : le comportement dépend d'un réglage global. Or le workspace est **inscriptible par l'enseignant** via `PUT /api/file` : c'est une entrée non fiable pour pi. Un réglage implicite n'est pas acceptable ici |
...
cmd := exec.CommandContext(ctx, piBin,
    ...
    "--no-context-files",            // n'absorbe pas un AGENTS.md posé dans le workspace
...
| 8c | `--no-context-files` | Aucun `AGENTS.md` ni `CLAUDE.md` du workspace n'entre dans le prompt (ce chargement échappe à la confiance projet) |
